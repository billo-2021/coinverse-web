...
export class GlobalRoutingService {
  ...
  constructor(@Inject(UserPrincipalStoreService) private readonly userPrincipalStore: UserPrincipalStoreService,
    ...
    this.verification$ = this.accountVerificationStore.accountVerification$
      .pipe(filter(
        (accountVerification): accountVerification is AccountVerification =>
          accountVerification !== null));

    this.login$ = this.userPrincipalStore.userLoggedIn$
  ...
  public start(): void {
    ...
    this.missingVerificationParamsRedirect$ = this.getMissingParamsRedirect(this.navigationService, this.accountVerificationStore);
  ...
  private getAutoLogout(userPrincipalStore: UserPrincipalStoreService,
                        navigationService: NavigationService): Subscription {
    return userPrincipalStore
      .userAccessCredentials$.pipe(
        filter((userCredentials):
        userCredentials is UserAccessCredentials => {
  ...
  private getVerificationRedirect(verification$: Observable<AccountVerification>,
                                  navigationService: NavigationService): Subscription {
    return verification$
      .pipe(tap(async (userVerification) => {
        if (!userVerification.isVerified) {
          await navigationService.to({path: webRoutesConfig.authentication.verifyAccount});
          return;
        }
