...
export class ManageTradesComponent extends BaseComponent {
  ...
  protected readonly columns: Key[] = ['id', 'amount', 'sourceWallet', 'destinationWallet',
  ...
  protected readonly request$ = combineLatest([this.pagination$])
    .pipe(switchMap(query =>
        this.tradeService.getTrades(...query).pipe(startWith(null))),
      shareReplay(1));

  protected trades$: Observable<readonly CurrencyTransactionResponse[]> = this.request$
