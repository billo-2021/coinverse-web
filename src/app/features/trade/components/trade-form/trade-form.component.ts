...
export class TradeFormComponent {
  ...
  public constructor(@Inject(LookupService) private readonly lookupService: LookupService,
                     @Inject(FormBuilder) private readonly formBuilder: FormBuilder) {
    this.form = this.getTradeForm(formBuilder);

    this.currencyPairOptions$ = lookupService.getAllCurrencyPairs()
      .pipe(map(currencyPairs => {
    ...
    combineLatest([this.currencyPairName$, this.currencyPairOptions$, this.currencyOptions$])
      .pipe(tap(([currencyPairName, currencyPairOptions]) => {
    ...
    this.filteredCurrencyOptions$ = combineLatest([this.currencyOptions$,
      this.form.controls['currencyPair'].valueChanges])
      .pipe(map(([currencyOptions, control]) => {
        ...
        const filteredCurrencyOptions = currencyOptions.filter(option =>
          option.code === baseCurrency.code || option.code == quoteCurrency.code);
