...
export class ManageCurrencyComponent {
  ...
  public constructor(@Inject(ActivatedRoute) private readonly route: ActivatedRoute,
                     ...
                     @Inject(AdministrationService) private readonly administrationService: AdministrationService,
                     @Inject(AlertService) private readonly alertService: AlertService) {

    this.currencyForm = this.getCurrencyForm(formBuilder,
      {id: 0, code: '', name: '', symbol: '', circulatingSupply: NaN});
  ...
  public getCurrencyForm(formBuilder: FormBuilder, cryptoCurrencyResponse: CryptoCurrencyResponse): FormGroup {
  ...
  public onSaveClicked(): void {
    ...
    if (mode === 'create') {
      this.administrationService.addNewCryptoCurrency({
        code,
        name,
        symbol,
        circulatingSupply
    ...
    this.administrationService.updateCryptoCurrency(code, {
      name,
      symbol,
      circulatingSupply
    }).pipe(tap(async () => {
