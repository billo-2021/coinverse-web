...
export class MenuService {
  ...
  constructor(@Inject(TuiBreakpointService) private readonly tuiBreakpoint: TuiBreakpointService,
  ...
  public get menuItems(): MenuItem[] {
    ...
    return filter.type === 'some' && this._menuItems.filter(menuItem => !filter.values.some(filter => (menuItem.text === filter || menuItem.link === filter)))
      || filter.type === 'all' && this._menuItems.filter(menuItem => !filter.values.every(filter => !(menuItem.text === filter || menuItem.link === filter)))
      || this._menuItems;
  }
