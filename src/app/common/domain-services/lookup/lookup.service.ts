...
export class LookupService {
  ...
  public getAllCountries(): Observable<CountryResponse[]> {
    return this.httpService.find<CountryDto[]>(this.getFullPath(this.ALL_COUNTRIES_PATH))
      .pipe(map((response) => response.map(countryDtoToCountryResponse)));
  }

  public getCountries(): Observable<PageResponse<CountryResponse>> {
    return this.httpService.find<PageResponse<CountryDto>>(this.getFullPath(this.COUNTRIES_PATH))
      .pipe(map((response) =>
        ({
  ...
  public getAllCurrencies(): Observable<CurrencyResponse[]> {
    return this.httpService.find<CurrencyDto[]>(this.getFullPath(this.ALL_CURRENCIES_PATH))
  ...
  public getCurrencies(): Observable<PageResponse<CurrencyResponse>> {
    return this.httpService.find<PageResponse<CurrencyDto>>(this.getFullPath(this.CURRENCIES_PATH))
      .pipe(map((response) =>
        ({
          count: response.count,
          total: response.total,
  ...
  public getCurrenciesByType(type: string, pageRequest: PageRequest): Observable<PageResponse<CurrencyResponse>> {
    const url = `${this.getFullPath(this.CURRENCIES_PATH)}?type=${type}&pageNumber=${pageRequest.page}&pageSize=${pageRequest.size}`;
  ...
  public getAllCryptoCurrencies(): Observable<CryptoCurrencyResponse[]> {
    ...
    return this.httpService.find<CryptoCurrencyDto[]>(url)
      .pipe(map((response) =>
        response.map(cryptoCurrencyDtoToCryptoCurrencyResponse))
      );
  }

  public getCryptoCurrencies(pageRequest: PageRequest): Observable<PageResponse<CryptoCurrencyResponse>> {
    const url = `${this.getFullPath(this.CRYPTO_CURRENCIES_PATH)}?pageNumber=${pageRequest.page}&pageSize=${pageRequest.size}`;

    return this.httpService.find<PageResponse<CryptoCurrencyDto>>(url)
      .pipe(map((response) => {
          return {
            count: response.count,
            total: response.total,
  ...
  public getCurrencyPairs(pageRequest: PageRequest): Observable<PageResponse<CurrencyPairResponse>> {
    const url = `${this.getFullPath(this.CURRENCY_PAIRS_PATH)}?pageNumber=${pageRequest.page}&pageSize=${pageRequest.size}`;
  ...
  public getPaymentMethods(pageRequest: PageRequest): Observable<PageResponse<PaymentMethodResponse>> {
    const url = `${this.getFullPath(this.ALL_PAYMENT_METHODS_PATH)}?pageNumber=${pageRequest.page}&pageSize=${pageRequest.size}`;
