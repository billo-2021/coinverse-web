...
export class UserPrincipalStoreService {
  ...
  constructor(@Inject(LocalStorageService) private readonly localStorageService: LocalStorageService,
              @Inject(JwtHelperService) private readonly jwtHelperService: JwtHelperService,
              @Inject(UserAccessCredentialsStoreService) private readonly userAccessCredentialsStoreService: UserAccessCredentialsStoreService) {
    ...
    this.userPrincipal$ = this._userPrincipal.asObservable().pipe(
      tap((userPrincipal) =>
        this.updateUserStorage(userPrincipal)));

    this.userLoggedIn$ = zip(this.userPrincipal$, this.userAccessCredentials$)
      .pipe(map(([userPrincipal, userCredentials]) => {
  ...
  public get isAdmin(): boolean {
    return !!this.userPrincipal && this.userPrincipal.roles.some(role => role.toLowerCase().includes('admin'));
  }

  public get isAdmin$(): Observable<boolean> {
    return this.userPrincipal$.pipe(
      map((userPrincipal) => !!userPrincipal && userPrincipal.roles.some(role => role.toLowerCase().includes('admin'))));
  }
  ...
  public isLoggedIn(): boolean {
    return this.isUserCredentialsValid(this.userPrincipal,
      this.userAccessCredentials);
  }
  ...
  public getTokenExpiryDate(token: string): Date {
    ...
    return DateTime.fromJSDate(tokenExpiryDate)
      .minus(TOKEN_EXPIRY_OFFSET)
      .toJSDate();
  }

  private isUserCredentialsValid(userPrincipal: UserPrincipal | null,
                                 userCredentials: UserAccessCredentials | null): boolean {
