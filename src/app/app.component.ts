...
export class AppComponent extends BaseComponent {
  // protected userLoggedIn$: Subscription;
  // protected userLoggedIn = false;
  protected user$ = this.userPrincipalStore.userPrincipal$;
  protected hasStarted = false;
  private readonly _viewModel$: Observable<AppViewModel>;

  public constructor(@Inject(NavigationService) private readonly navigationService: NavigationService,
    ...
    this._viewModel$ = combineLatest([this.userPrincipalStore.userLoggedIn$, this.userPrincipalStore.userPrincipal$]).pipe(
      ...
      tap(({isAdmin, isMenuShown}) => {
        this.menuService.menuFilter = isAdmin && {type: 'none'}
          || {type: 'some', values: ['Currencies', 'Users']};
